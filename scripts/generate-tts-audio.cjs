/**
 * Script CommonJS: Tạo tự động các file âm thanh ngắn (.mp3) chuẩn ngữ âm Tiếng Việt
 * Sử dụng Microsoft Edge TTS (Voice: vi-VN-HoaiMyNeural - Giọng nữ miền Bắc chuẩn sư phạm)
 *
 * Cách chạy:
 *   node scripts/generate-tts-audio.cjs
 */
...
const DELAY_BETWEEN_REQUESTS_MS = 180;
...
async function main() {
  ...
  console.log('🎤 KHỞI ĐỘNG BỘ TẠO ÂM THANH NGỮ ÂM TIẾNG VIỆT LỚP 1');
  ...
  console.log(`📄 Đã tạo file mục lục: raw-audio/manifest.json`);
  console.log('='.repeat(70));
}
