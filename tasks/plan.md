...
#### Checkpoint 13: Âm Học Chuẩn Xác & Giao Diện Tối Giản Hoàn Hảo [ĐÃ ĐẠT]
- [x] 28 phụ âm phát âm chuẩn xác không tạp âm ngoại lai.
- [x] Giao diện sạch sẽ, thân thiện với trẻ 5-6 tuổi.
- [x] 100% tests PASS (119/119), build sạch sẽ.
...
#### Mục tiêu & Căn cứ:
- Chuẩn hóa công thức đánh vần cho từ có thanh điệu thành chuỗi 5 bước: `[âm đầu] - [vần không dấu] - [tiếng thanh ngang] - [dấu thanh] - [tiếng hoàn chỉnh]`. Ví dụ từ "lớp" là `l - ơp - lơp - sắc - lớp`.
- Xây dựng Sân khấu đánh vần phía TRÊN bảng bài đọc (`KidReaderBoard`) trong chế độ `spelling` (Phương án 1).
- Hiển thị hiệu ứng Karaoke thời gian thực theo từng bước phát âm (`activeStepIndex`), bé chạm vào từng thẻ để nghe lại riêng từng mẩu âm.
...
- [x] **Task 14.1: Cập nhật Parser Ngữ Âm chuẩn hóa 5 bước cho mọi âm tiết có thanh**
  ...
  - Từ "lớp" trả về: `formula: ['l', 'ơp', 'lơp', 'sắc', 'lớp']`, `formulaText: 'l - ơp - lơp - sắc - lớp'`.
  - Đồng bộ các từ khép tắc có thanh khác ("bắt", "học", "hát", "vịt", "sách") sang 5 bước.
...
- [x] **Task 14.3: Xây dựng Component `PhonicsKaraokeStage.tsx`**
  - Thẻ bài Montessori xúc giác lớn ($\ge 56\text{px}$), bố cục chuỗi phân rã ngữ âm.
  - Karaoke Highlight: bừng sáng viền vàng hổ phách, scale-110, icon loa chuyển động nhịp nhàng theo `activeStepIndex`.
  - Tương tác 1-chạm: Bé nhấn vào từng thẻ riêng biệt trên sân khấu để nghe lại âm thanh của riêng bước đó.
